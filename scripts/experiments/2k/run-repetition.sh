...
echo "Write percent ${write_percent}"
...
do
  ...
  parallel-ssh \
    ...
    -l ruxu \
    "rm -rf ~/log; mkdir -p ~/log; nohup ~/libmemcached-1.0.18/clients/memaslap -s ${vm_11_private}:8000 -T ${num_clients} -c ${num_clients} -w 1K -o 0.9 -t 60s -S 1s -F ~/scripts/experiments/2k/write${write_percent}-message${message_size}.cfg 1 > ~/log/client.log 2>&1 &"
