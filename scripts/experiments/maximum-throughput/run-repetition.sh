...
do
  ...
  parallel-ssh \
    -i \
    -H "${vm_6_public} ${vm_7_public} ${vm_8_public} ${vm_9_public} ${vm_10_public}" \
    -l ruxu \
  ...
  parallel-ssh \
    ...
    -H "${vm_11_public}" \
    -l ruxu \
    "rm -rf ~/log; mkdir -p ~/log; nohup java -jar ~/middleware-ruxu.jar -l ${vm_11_private} -p 8000 -m ${vm_6_private}:11212 ${vm_7_private}:11212 ${vm_8_private}:11212 ${vm_9_private}:11212 ${vm_10_private}:11212 -t ${thread_pool_size} -r 1 > /dev/null 2>&1 &"
  ...
  parallel-ssh \
    ...
    -l ruxu \
    "rm -rf ~/log; mkdir -p ~/log; nohup ~/libmemcached-1.0.18/clients/memaslap -s ${vm_11_private}:8000 -T ${num_clients} -c ${num_clients} -w 1K -t 60s -S 1s -F ~/scripts/experiments/maximum-throughput/maximum-throughput.cfg 1 > ~/log/client.log 2>&1 &"
